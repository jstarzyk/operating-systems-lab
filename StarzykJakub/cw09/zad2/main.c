...
void *producer(void *args) {
    ...
    while (true) {
        res = sem_wait(&empty_count);
        if (res == -1) error(EXIT_FAILURE, errno, "sem_wait");
        if (verbose) printf("[P-%ld] DEC empty_count\n", self);
        ...
        res = sem_post(&fill_count);
        if (res == -1) error(EXIT_FAILURE, errno, "sem_post");
        if (verbose) printf("[P-%ld] INC fill_count\n", self);
...
void *consumer(void *args) {
    ...
    while (true) {
        res = sem_wait(&fill_count);
        if (res == -1) error(EXIT_FAILURE, errno, "sem_wait");
        if (verbose) printf("[C-%ld] DEC fill_count\n", self);
        ...
        res = sem_post(&empty_count);
        if (res == -1) error(EXIT_FAILURE, errno, "sem_post");
        if (verbose) printf("[C-%ld] INC empty_count\n", self);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    res = sem_init(&fill_count, 0, 0);
    if (res != 0) error(EXIT_FAILURE, errno, "sem_init");
    res = sem_init(&empty_count, 0, (unsigned int) n);
    if (res != 0) error(EXIT_FAILURE, errno, "sem_init");
